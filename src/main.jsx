...
const SAFE_BACKUP_KEY = "tkpa_cahier_safe_last_good";
const SAFE_BACKUP_LIST_KEY = "tkpa_cahier_safe_versions";
...
function countBusinessItems(data){
  return {
    users: Array.isArray(data?.users) ? data.users.length : 0,
    fiches: Array.isArray(data?.fiches) ? data.fiches.length : 0,
    devis: Array.isArray(data?.devis) ? data.devis.length : 0,
    archives: Array.isArray(data?.archivesJour) ? data.archivesJour.length : 0,
  };
}

function hasBusinessData(data){
  const c = countBusinessItems(data);
  return c.users > 0 || c.fiches > 0 || c.devis > 0 || c.archives > 0;
}

function isDangerouslyEmpty(next, current){
  const n = countBusinessItems(next);
  const c = countBusinessItems(current);
  const currentHasWork = c.fiches > 0 || c.devis > 0 || c.archives > 0;
  const nextHasNoWork = n.fiches === 0 && n.devis === 0 && n.archives === 0;
...
function loadSafeSnapshot(){
  try{
    const raw = localStorage.getItem(SAFE_BACKUP_KEY);
    if(!raw) return null;
    const parsed = JSON.parse(raw);
    return parsed?.payload ? normalizeState(parsed.payload) : null;
  }catch{
    return null;
  }
}

function mergePayloadSafe(base, incoming){
  ...
  const scoreObject = (x) => {
    try {
      let score = 0;
      if(Array.isArray(x?.pieces)) score += x.pieces.length * 10;
      if(Array.isArray(x?.lignes)) score += x.lignes.length * 10;
      if(Array.isArray(x?.propositions)) score += x.propositions.length * 5;
  ...
  const mergeArray = (oldArr=[], newArr=[]) => {
    ...
    newArr.forEach((x) => {
      if(!x || !x.id) return;
      const previous = map.get(x.id);
      if(!previous) {
        map.set(x.id, x);
      } else {
        // On garde la version la plus complète pour éviter qu'un poste salarié ancien écrase une fiche/devis détaillé.
        map.set(x.id, scoreObject(x) >= scoreObject(previous) ? {...previous, ...x} : {...x, ...previous});
      }
    });
  ...
  const users = mergeArray(b.users, i.users);
  return {
    users: users.length ? users : initialUsers,
    fiches: mergeArray(b.fiches, i.fiches),
    devis: mergeArray(b.devis, i.devis),
    archivesJour: mergeArray(b.archivesJour, i.archivesJour),
    lastClosureDate: i.lastClosureDate || b.lastClosureDate || "",
    lastAutoBackupAt: i.lastAutoBackupAt || b.lastAutoBackupAt || ""
  };
}
...
async function saveCloud(data){
  ...
  const localClean = normalizeState(data);
  const safe = loadSafeSnapshot();

  if(isDangerouslyEmpty(localClean, safe || {})){
    console.error("Sauvegarde bloquée : tentative d'écrasement vide détectée.");
    throw new Error("Sauvegarde bloquée : données vides détectées.");
  }

  // Sécurité multi-PC :
  // Avant d'écrire, on relit Supabase et on fusionne.
  // Comme ça, un salarié ou un autre PC ne peut pas écraser les devis/fiches déjà présents.
  let latestCloud = null;
  try {
    latestCloud = await loadCloudState();
  } catch(e) {
    console.warn("Impossible de relire Supabase avant sauvegarde, copie locale conservée", e);
  }

  const cloudClean = normalizeState(latestCloud || {});
  let merged = mergePayloadSafe(cloudClean, localClean);

  // Protection supplémentaire :
  // Si Supabase contient plus de fiches/devis que la version locale, on garde Supabase.
  const cloudCount = countBusinessItems(cloudClean);
  const localCount = countBusinessItems(localClean);
  const mergedCount = countBusinessItems(merged);

  if(cloudCount.fiches > localCount.fiches || cloudCount.devis > localCount.devis || cloudCount.archives > localCount.archives){
    merged = mergePayloadSafe(localClean, cloudClean);
  }

  // Blocage final si le résultat devient plus pauvre que la dernière bonne copie.
  if(safe && isDangerouslyEmpty(merged, safe)){
    console.error("Sauvegarde bloquée : fusion dangereuse détectée.");
    throw new Error("Sauvegarde bloquée : fusion dangereuse.");
  }
...
function autoDownloadBackup(data, backup){
  try{
    const key = `${backup.date}-${String(backup.heure || "").slice(0,5)}`;
...
function createAutoBackupStateWithDownload(data, shouldDownload=false){
  ...
  const sameHalfHourExists = archives.some((a) =>
    a.type === "auto" &&
    a.date === backup.date &&
    String(a.heure || "").slice(0, 2) === String(backup.heure || "").slice(0, 2) &&
    Math.floor(Number(String(a.heure || "00:00").slice(3,5) || 0) / 30) === Math.floor(Number(String(backup.heure || "00:00").slice(3,5) || 0) / 30)
  );

  if(sameHalfHourExists) {
    if(shouldDownload) autoDownloadBackup(data, backup);
    return data;
  }
  ...
  if(shouldDownload) autoDownloadBackup(next, backup);
  return next;
}
...
function sortedDateKeys(grouped){
  return Object.keys(grouped).sort((a,b) => String(b).localeCompare(String(a)));
}
...
function App(){
  ...
  useEffect(()=>{
    ...
    async function start(){
      ...
      try{
        ...
        if(cancelled) return;
        let clean=cloud?normalizeState(cloud):normalizeState(loadState());
        const localCurrent = normalizeState(loadState());
        const safeCurrent = loadSafeSnapshot();

        if(isDangerouslyEmpty(clean, localCurrent) && safeCurrent){
          clean = mergePayloadSafe(clean, safeCurrent);
          await saveCloudState(clean);
          setSyncStatus("Restauration sécurité locale appliquée");
        } else if(cloud && hasBusinessData(localCurrent) && isDangerouslyEmpty(clean, localCurrent)){
          clean = mergePayloadSafe(clean, localCurrent);
          await saveCloudState(clean);
    ...
    const ch = subscribeCloudState((next)=>{
      ...
      setData((current)=>{
        if(isDangerouslyEmpty(incoming, current)){
          const safe = loadSafeSnapshot();
          const restored = safe ? mergePayloadSafe(incoming, safe) : current;
          saveCloud(restored).catch(console.error);
  ...
  useEffect(()=>{
    const interval = setInterval(() => {
      setData((current) => {
        const user = loadSession(current.users);
        if(user?.role !== "admin") return current;

        const next = createAutoBackupStateWithDownload(current, true);
        if(next === current) return current;

        lastSaveRef.current = Date.now();
        saveLocal(next);
        saveCloud(next)
          .then(()=>setSyncStatus("Sauvegarde automatique 30 min téléchargée"))
          .catch(e=>{ console.error(e); setSyncStatus("Erreur sauvegarde automatique - copie téléchargée conservée"); });
  ...
  function commit(next){
    const current = normalizeState(data);
    let clean = normalizeState(next);

    if(isDangerouslyEmpty(clean, current)){
      alert("Sécurité : sauvegarde vide bloquée pour éviter une perte de données.");
      setSyncStatus("Sauvegarde vide bloquée");
      return;
    }

    clean = mergePayloadSafe(current, clean);
    if(hasBusinessData(clean)) saveSafeSnapshot(clean, "commit");
  ...
  if(!currentUser){
    return <div className="login-page"><div className="login-card"><div className="brand"><img src={logo}/><div><h1>{ENTREPRISE.nom}</h1><p>Cahier salarié synchronisé</p></div></div><form onSubmit={connect} className="login-form"><label>Identifiant<input value={login.identifiant} onChange={e=>setLogin({...login,identifiant:e.target.value})}/></label><label>Mot de passe<input type="password" value={login.motDePasse} onChange={e=>setLogin({...login,motDePasse:e.target.value})}/></label><button className="primary full">Connexion</button></form></div></div>;
  }
  ...
  return <div className="app">
    <aside>
      ...
      <nav>
        ...
        <button className={active==="cahier"?"on":""} onClick={()=>setActive("cahier")}><Archive/>Cahier salarié</button>
    ...
    <main>
      {active==="dashboard"&&<section>
        <div className="hero" style={{backgroundImage:`linear-gradient(90deg, rgba(7,21,60,.92), rgba(7,21,60,.55)), url(${hero})`}}><div><h1>Cahier salarié</h1><p>Demandes en attente, recherches réalisées, envoi vers devis client et sauvegarde journalière à 19h.</p><button className="primary" onClick={newFiche}><Plus/>Nouvelle demande</button></div></div>
      ...
      {active==="cahier"&&<section><Header title="Cahier salarié" subtitle={currentUser.role==="admin"?"Admin : tous les cahiers. Recherche ancienne par plaque, VIN, nom, téléphone ou référence.":"Salarié : ton cahier uniquement. Exception : retrouver une fiche par plaque."}/><div className="toolbar"><div className="search"><Search/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Recherche plaque, VIN, nom, téléphone..."/></div>{currentUser.role==="admin"?<select value={selectedUserId} onChange={e=>setSelectedUserId(e.target.value)}><option value="all">Tous les cahiers</option>{data.users.map(u=><option key={u.id} value={u.id}>Cahier de {u.nom}</option>)}</select>:<div className="locked-filter">Mon cahier</div>}<button className="primary" onClick={newFiche}><Plus/>Nouvelle</button><button className={showAllFiches ? "active-soft" : ""} onClick={()=>setShowAllFiches(!showAllFiches)}><Archive/>{showAllFiches ? "Afficher jour actuel" : "Afficher toutes les anciennes fiches"}</button></div>{showAllFiches && <div className="info-banner">Mode historique activé : les anciennes fiches sont classées par journée séparée. Tu peux aussi rechercher par plaque, VIN ou nom.</div>}{showAllFiches ? (
              <div className="archive-day-list">
                {sortedDateKeys(groupFichesByDate(visibleFiches)).map((dateKey) => (
                  <div className="archive-day-block" key={dateKey}>
                    <div className="archive-day-head">
                      <h3>Journée du {dateKey}</h3>
                      <span>{groupFichesByDate(visibleFiches)[dateKey].length} fiche(s)</span>
                    </div>
                    <div className="cards">
                      {groupFichesByDate(visibleFiches)[dateKey].map(f=>(
                        <FicheCard key={f.id} f={f} currentUser={currentUser} canEdit={canEdit(f)} canDelete={canDelete()} open={()=>{setEditing(f);setOpenPieceId(f.pieces?.[0]?.id||"");setActive("edition")}} preview={()=>setPreview(f)} del={()=>{if(confirm("Supprimer ?"))commit({...data,fiches:data.fiches.filter(x=>x.id!==f.id)})}} />
                      ))}
                    </div>
                  </div>
                ))}
              ...
              <div className="cards">{visibleFiches.map(f=><FicheCard key={f.id} f={f} currentUser={currentUser} canEdit={canEdit(f)} canDelete={canDelete()} open={()=>{setEditing(f);setOpenPieceId(f.pieces?.[0]?.id||"");setActive("edition")}} preview={()=>setPreview(f)} del={()=>{if(confirm("Supprimer ?"))commit({...data,fiches:data.fiches.filter(x=>x.id!==f.id)})}} />)}</div>
            )}</section>}
      ...
      {active==="sauvegardes"&&<section>
        ...
        <div className="actions top-actions">
          ...
          <button onClick={()=>commit(createAutoBackupStateWithDownload(data, currentUser?.role==="admin"))}><Archive/>Créer une sauvegarde automatique maintenant</button>
        </div>
...
function Editor({ editing, setEditing, openPieceId, setOpenPieceId, saveFiche, sendToDevis, setPreview, cancel }){
  ...
  function pasteImageProp(event,pid,idx){
    const items = event.clipboardData?.items || [];
    const imageItem = Array.from(items).find((item)=>item.type && item.type.startsWith("image/"));
    if(!imageItem) return;
    event.preventDefault();
    const file = imageItem.getAsFile();
    if(file) imageProp(file,pid,idx);
  }
...
<label>Disponible pour quand<input value={pr.disponibleQuand||""} onChange={e=>updateProp(piece.id,idx,{disponibleQuand:e.target.value})} placeholder="Ex : aujourd'hui, demain, 48h..."/></label></div><div className="image-line proposition-image-line">
                {pr.image?<img src={pr.image}/>:<div className="empty-img"><ImagePlus/>Image réf.</div>}
                <div className="paste-image-zone" tabIndex="0" onPaste={(e)=>pasteImageProp(e,piece.id,idx)}>
                  <ImagePlus/>
                  <b>Coller image</b>
                  <small>Copie une image puis clique ici et fais Ctrl+V</small>
                </div>
                <label className="upload"><ImagePlus/>Ajouter fichier<input type="file" accept="image/*" onChange={e=>e.target.files?.[0]&&imageProp(e.target.files[0],piece.id,idx)}/></label>
                {pr.image&&<button className="danger" onClick={()=>updateProp(piece.id,idx,{image:""})}><Trash2/>Retirer</button>}
              </div></div>)}</div></div>}</div><div className="bottom-actions"><button onClick={cancel}><X/>Retour</button><button onClick={()=>setPreview(editing)}><Eye/>Consultation détaillée du devis</button><button className="primary" onClick={()=>saveFiche({...editing,statut: isRechercheTerminee(editing) ? "realise" : "en_cours"})}><Save/>Enregistrer dans le cahier</button><button onClick={()=>sendToDevis({...editing,statut:"realise"})}><Send/>Envoyer vers devis</button></div></div></section>;
}
